...
- **Role positioning**: the four target tracks are iOS, full-stack, backend, and AI engineer. Headlines and meta say `iOS, full-stack, backend, and AI roles` (in that order). The For Recruiters target-roles row spells out the role titles more concretely (`iOS engineer, full-stack / software engineer, backend engineer, and AI engineer`). Do not reintroduce "primary iOS / secondary backend" framing. "New-grad" appears exactly once on the page (in `ForRecruiters.tsx`'s target-roles row) - keep it that way; the graduation date carries the rest of the signal.
